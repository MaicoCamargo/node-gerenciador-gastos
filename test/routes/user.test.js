...
const URL_USER = `${MAIN_ROUTE}/user`;
 ...
 */
  ...
  const response = await request(app).post(AUTH_ROUTE).send({ mail: mailAuth, passwd });
...
test('Deve listar todos usuarios', () => {
  return request(app).get(URL_USER).set('Authorization', `bearer ${token}`).then((response) => {
    expect(response.status).toBe(200);
...
test('Deve criar um usuario com sucesso', () => {
  return request(app).post(URL_USER).send({ nome: 'Eder Camargo', mail, passwd: '1234' }).set('Authorization', `bearer ${token}`)
    .then((response) => {
...
test('Não deve criar um usuario sem nome', () => {
  return request(app).post(URL_USER).send({ mail: 'test@email.com', passwd: '1234' }).set('Authorization', `bearer ${token}`)
    .then((response) => {
...
test('Não deve criar um usuario sem email', async () => {
  const result = await request(app).post(URL_USER).send({ nome: 'ana pa', passwd: '1234' }).set('Authorization', `bearer ${token}`);
...
test('Não deve criar um usuario sem senha', (done) => {
  request(app).post(URL_USER).send({ nome: 'ana pa', mail: 'test@email.com' }).set('Authorization', `bearer ${token}`)
    .then((result) => {
      expect(result.status).toBe(400);
...
test('Não deve cadastrar usuario com email já inserido', async () => {
  const result = await request(app).post(URL_USER).set('Authorization', `bearer ${token}`).send({ nome: 'Eder Camargo', mail, passwd: '1234' });
...
test('Deve criar usuário com senha criptografada', async () => {
  const passwd = '123456';
  const response = await request(app).post(URL_USER)
